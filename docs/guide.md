...
A `CLAUDE.md` file gives Claude full context about your project —
what it is, how to build and deploy it, the git workflow, and important
rules. Claude reads this automatically on every session start.

Create one at the project root and fill in your real values where indicated:

```markdown
# <App Name> — ServiceNow Fluent Scoped App

## Project Overview
- **App name:** <Your App Name>
- **Scope:** <x_snc_your_scope>
- **Instance alias:** <your-alias>  (set via `now-sdk auth --add <alias>`)
- **Instance URL:** https://<your-instance>.service-now.com
- **SDK version:** 4.4.x (`@servicenow/sdk`)
- **Primary editor:** Claude Code + ServiceNow IDE

## What is Fluent DSL?
This app is written in Fluent DSL — a TypeScript-like language that compiles
to ServiceNow records. Source lives in `src/`. Never edit `dist/` directly;
it is generated by `now-sdk build` and deployed by `now-sdk install`.

## Project Structure
```
<app-root>/
├── src/                    # Fluent DSL source — edit here
├── dist/                   # Compiled output — do not edit (generated)
├── now.config.json         # SDK project config (scope, name, tsconfig path)
├── now.dev.mjs             # Dev server entry point
├── now.prebuild.mjs        # Prebuild script
├── package.json            # npm config and dependencies
├── .eslintrc               # Linting rules
...
| Command | Purpose |
|---------|---------|
| `npm install` | Install dependencies (run once after init, and if package.json changes) |
| `now-sdk build` | Compile Fluent DSL → `dist/` — **required before every install** |
| `now-sdk install --alias <alias>` | Deploy `dist/` to the PDI |
| `now-sdk download src/` | Pull app metadata FROM instance → local (only when instance has changes) |
| `now-sdk auth --add <alias>` | Authenticate SDK to a new instance |

## Development Loop

**Always follow this order:**
1. Edit files in `src/`
2. `now-sdk build` — fix any errors before continuing
3. `now-sdk install --alias <alias>` — on success, prints a direct URL to the app
4. Verify the change in the PDI browser
5. Commit to a feature branch and open a PR

**`now-sdk download src/` is NOT part of the normal loop.**
Use it only when changes were made directly on the instance (e.g. in Studio)
and you need to pull them back to local.
...
## MCP
The `foundry` MCP server is registered at user scope:
```bash
...
## Rules & Gotchas
- `npm install` must be run after `now-sdk init` before anything else
- `now-sdk build` must succeed before `now-sdk install` — install reads `dist/`
- `now-sdk install` requires `--alias <name>` — it will fail without it
- Never commit `dist/`, `.snc/`, auth files, or any credentials
- `now.config.json` is the SDK project config (older docs may say `now-sdk.json` — ignore that)
- Deprecation warnings from `npm install` are normal — they come from SDK dependencies
```
